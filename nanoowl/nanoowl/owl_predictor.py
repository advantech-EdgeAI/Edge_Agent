...
class OwlPredictor(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.device = device
        # self.model = OwlViTForObjectDetection.from_pretrained(model_name).to(self.device).eval() #v1
        # self.processor = OwlViTProcessor.from_pretrained(model_name) #v1
        #from nano_llm import Plugin
        #Plugin.print_memory_usage()
    ...
    def encode_text(self, text: List[str]) -> OwlEncodeTextOutput:
        ...
        #text_outputs = self.model.owlvit.text_model(input_ids, attention_mask) #v1
        text_outputs = self.model.owlv2.text_model(input_ids, attention_mask)
        text_embeds = text_outputs[1]
        #text_embeds = self.model.owlvit.text_projection(text_embeds) #v1
    ...
    def encode_image_torch(self, image: torch.Tensor) -> OwlEncodeImageOutput:
        ...
        #vision_outputs = self.model.owlvit.vision_model(image) #v1
        vision_outputs = self.model.owlv2.vision_model(image)
        last_hidden_state = vision_outputs[0]
        #image_embeds = self.model.owlvit.vision_model.post_layernorm(last_hidden_state) #v1
